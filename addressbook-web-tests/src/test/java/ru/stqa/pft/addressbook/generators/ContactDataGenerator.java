...
public class ContactDataGenerator {
  ...
  private void saveAsJson(List<ContactData> contacts, File file) throws IOException {
    Gson gson = new GsonBuilder().registerTypeAdapter(ContactData.class, new ContactDataAdapter())
  ...
  private void saveAsCsv(List<ContactData> contacts, File file) throws IOException {
    ...
    for (ContactData contact : contacts) {
      writer.write(String.format("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n", contact.getFirstName(), contact.getLastName(),
              contact.getMiddleName(), contact.getPhoneHome(), contact.getPhoneMobile(),
              contact.getPhoneWork(), contact.getNick(), contact.getAddress(), contact.getEmail(), contact.getPhoto()));
    }
    writer.close();
  }
  ...
  private List<ContactData> generateContacts(int count) {
    ...
    for (int i = 0; i < count; i++) {
      contacts.add(new ContactData().withFirstName(String.format("LastName%s", i)).withLastName(String.format("FirstName%s", i))
